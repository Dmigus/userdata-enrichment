...
# Брокер
Для передачи запросов на обогащение используется очередь `requests`. Если тело сообщения не является допустимым, оно переправляется в DLQ `incorrect.requests`.
