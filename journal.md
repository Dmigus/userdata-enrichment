...
Этот вариант мы реализуем. Необходимость согласованности данных в БД и очереди заставляет нас сделать транзакцию. Наиболее подходящий вариант, по мнению автора, использовать шаблон Outbox. В зависимости от выбранных технологий, транзакция может быть распределённой, если очередь и БД используют не одно и то же хранилище. Автор предлагает ограничится реализацией очереди в том же хранилище, где и БД, так как использование распределённых транзакций требует развёртывания ещё нескольких компонентов (например, Debezium + Kafka).
...
## Очередь
Создадим отдельную таблицу outbox. Она содержит все необработанные запросы на обогащение в виде тройки (name, surname, patronymic). 
Добавим индекс по дате добавления записи, чтобы быстро искать старые.